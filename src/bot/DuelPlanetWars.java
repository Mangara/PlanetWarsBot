...
public class DuelPlanetWars {
    ...
    private void computeNearbyGrowth(Planet planet) {
        double nearbyGrowth = 0;

        for (Planet p : planets) {
            if (p != planet) {
                nearbyGrowth += p.getGrowthRate() / (double) getDistance(p, planet);
            }
        }

        planet.setNearbyGrowth(nearbyGrowth);
    }
