...
public class MyBot {
    ...
    private static class Attack implements Comparable<Attack> {
        ...
        public void computeScore(Prediction2[] predictions) {
            ...
            if ((defensive || predictions[target.getPlanetID()].getOwnerAt(takeOverTurn) == 0) && shipsAvailable < shipsNeeded + antiSnipeShipsNeeded) {
                // Don't get sniped
                score = 0;
            } else {
                score = (nResultingShips - shipsAvailable) / ((double) (shipsNeeded + antiSnipeShipsNeeded) * takeOverTurn);
            }
        ...
        for (Planet planet : pw.getClosestPlanets(target)) {
            ...
            if (dist <= turn) {
                if (planet.getOwner() == 1) { // TODO: check predictions to coordinate with planets that will be mine?
                    int surplus = getAttackSurplus(pw, predictions, planet, turn - dist);
