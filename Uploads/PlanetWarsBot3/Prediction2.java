...
public class Prediction2 {
    ...
    public Pair<Integer, Integer> getLastHostileTakeOver() {
        ...
        while (it.hasPrevious()) {
            ...
            if (d.owner == 1) {
                it.next(); // returns d
                DataPoint afterTakeOver = it.next();

                return new Pair<Integer, Integer>(afterTakeOver.turn, afterTakeOver.numShips);
            }
        }

        // We never owned this planet
        return null;
    }
    ...
    public void computePredictions() {
        ...
        if (MyBot.DEBUG_PREDICTIONS) {
            ...
            if (forces.size() > 0) {
                ...
                for (Entry<Integer, Force> entry : forces.entrySet()) {
                    log.println("Turn " + entry.getKey() + ": " + entry.getValue());
                }
            }
            log.flush();
        }
        ...
        if (MyBot.DEBUG_PREDICTIONS) {
            ...
            log.print("New prediction for planet " + planet + ": ");

            int n = 20;

            for (int i = 0; i < n - 1; i++) {
                log.print(getNumShipsAt(i, true) + ", ");
            }

            log.println(getNumShipsAt(n - 1, true));
            log.flush();
        }
    }
